...
def proxy():
    UseProxy = input("[~] Do you want to use a proxy Y/n: ")
    if UseProxy == 'Y' or UseProxy == 'y':
        proxy = input("[!] Enter Proxy | ex: http://IP:Port \n > ")
...
def server():
    ...
    connection = urllib.request.urlopen("https://www.bing.com/search?q=ip:"+ip+" php?id=", None, headers)
    ...
    print("\n[#] Done !")
def Google():
    dork = input(' > Enter Dork | ex: inurl:".php?id=":\n > ')
    Results = eval(input(" > How many results do you want?\n > "))
